...
resource "aws_sns_topic" "budget_alert" {
  name = "ai-multi-agent-budget-alert"
}
...
resource "aws_budgets_budget" "cost_limit" {
  name         = "ai-multi-agent-cost-limit"
  budget_type  = "COST"
  ...
  cost_filter {
    name   = "TagKeyValue"
    values = ["user:service$ai-multi-agent"]
  }
...
resource "aws_lambda_function" "budget_killer" {
  function_name    = "ai-multi-agent-budget-killer"
...
resource "aws_iam_role" "budget_killer_role" {
  name = "ai-multi-agent-budget-killer-role"
...
resource "aws_iam_role_policy" "budget_killer_policy" {
  name = "ai-multi-agent-budget-killer-policy"
