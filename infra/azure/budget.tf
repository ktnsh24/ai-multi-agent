...
resource "azurerm_automation_account" "budget_killer" {
  name                = "multi-agent-budget-killer"
...
resource "azurerm_automation_runbook" "kill_resources" {
  name                    = "multi-agent-kill-resources"
  location                = azurerm_resource_group.multi_agent.location
  resource_group_name     = azurerm_resource_group.multi_agent.name
  ...
  content = <<-POWERSHELL
    param([string]$ResourceGroupName = "${azurerm_resource_group.multi_agent.name}")
...
resource "azurerm_consumption_budget_resource_group" "cost_limit" {
  name              = "multi-agent-cost-limit"
  resource_group_id = azurerm_resource_group.multi_agent.id
