#!/usr/bin/env bash
# =============================================================================
# 🚀 Cloud Lab Runner — ai-multi-agent
# =============================================================================
...
start_server() {
    log "🖥️  Starting ai-multi-agent server on port $PORT..."
    if $DRY_RUN; then log "[DRY RUN] poetry run start"; return 0; fi
    (
        cd "$REPO_DIR"
        poetry run uvicorn src.main:create_app --factory --host 0.0.0.0 --port "$PORT" \
